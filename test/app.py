...
model1 = torch.load('./models/7emotions_model.pt', map_location=device)                                       # 전체 모 델을 통째로 불러옴, 클래스 선언 필수.
model1.load_state_dict(torch.load('./models/7emotions_model_state_dict.pt', map_location=device))             # state_dict를 불러 온 후, 모델에 저장.

checkpoint = torch.load('./models/7emotions_all.tar', map_location=device)                                    # dict 불 러오기.
...
def SolveTheResult(text):                                                       #들어가는 text는 str형이며, \n이 없도록 주의해주세요.
    ...
    for i in range(len(emotions)):                                                #emotions의 각각의 감정을 총합과 나누 어 %로 나타낸다.
        try:                                                                        #감정의 값이 0일 경우를 위한 예외처 리
